...
main() async {
  ...
  $.onValueChange("/Page_Color").listen((ValueUpdate? update) async { // Wait for background color changes.
    String? color = update!.value as String?;
    try {
      color = "#${int.parse(update.value.toString()).toRadixString(16).padLeft(6, '0')}";
    } catch (e) {}
    bodyElement
  ...
  $.onValueChange("/Text_Color").listen((ValueUpdate update) async { // Wait for text color changes.
    String color = update.value as String;
    try {
      color = "#${int.parse(update.value as String).toRadixString(16).padLeft(6, '0')}";
    } catch (e) {}
    textElement
      ?..style.color = color
  ...
  $.onValueChange("/Page_Gradient").listen((ValueUpdate update) {
    ...
    String x = update.value as String;
    bodyElement
