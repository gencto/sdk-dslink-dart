...
void main() async {
  ...
  print('💡 In production, configure logging appropriately for your environment.');
}
...
Future<void> demonstrateStructuredLogging() async {
  final logger = DsLogger.getLogger('StructuredLogging');
...
Future<void> demonstratePerformanceLogging() async {
  ...
  logger.info('Starting complex data processing operation...');
  
  final stopwatch = Stopwatch()..start();
  
  // Simulate work with multiple phases
  await simulateProcessingPhase('Data loading', Duration(milliseconds: 120));
  await simulateProcessingPhase('Data validation', Duration(milliseconds: 80));
  await simulateProcessingPhase('Data transformation', Duration(milliseconds: 200));
  await simulateProcessingPhase('Data persistence', Duration(milliseconds: 150));
...
Future<void> simulateProcessingPhase(String phaseName, Duration duration) async {
...
Future<void> demonstrateRequestResponseLogging() async {
  ...
  final logger = DsLogger.getLogger('RequestResponse');
  logger.info('Completed ${completedRequests.length} correlated requests');
}
...
class DataProcessor with LoggerMixin {
  Future<void> processData(List<String> items) async {
    ...
    try {
      ...
      stopwatch.stop();
      logInfo('Data processing completed successfully in ${stopwatch.elapsedMilliseconds}ms');
...
class NetworkClient with LoggerMixin {
  ...
  Future<Map<String, dynamic>> fetchData(String url) async {
    final requestId = 'net_${++_requestCount}_${DateTime.now().millisecondsSinceEpoch}';
    final stopwatch = Stopwatch()..start();
...
class CacheManager with LoggerMixin {
  ...
  Future<void> cacheItem(String key, dynamic value) async {
    final stopwatch = Stopwatch()..start();
  ...
  T? getItem<T>(String key) {
    ...
    try {
      logDebug('Retrieving item with key: $key');
      ...
      stopwatch.stop();
      logError('Failed to retrieve item: $key', error, stackTrace);
      return null;
    }
  }
}