...
void main() {
  group('Logger Tests', () {
    ...
    test('LoggerMixin provides logging capabilities', () {
      ...
      expect(() => testClass.logWarning('Test warning message'), returnsNormally);
    ...
    test('Structured logging methods work correctly', () {
      ...
      expect(() => DsLogger.logPerformance(
        'Test operation',
        Duration(milliseconds: 100),
        context: {'key': 'value'},
      ), returnsNormally);
      
      expect(() => DsLogger.logConnection(
        'test_event',
        'test_endpoint',
        metadata: {'protocol': 'websocket'},
      ), returnsNormally);
      
      expect(() => DsLogger.logRequest(
        'GET',
        '/test/path',
        requestId: 'test-123',
      ...
      expect(() => DsLogger.logError(
        'Test error',
        Exception('Test exception'),
        context: {'operation': 'test'},
      ), returnsNormally);
      
      expect(() => DsLogger.logSecurity(
        'Authentication',
        'User login attempt',
        context: {'userId': 'test-user'},
      ), returnsNormally);
      
      expect(() => DsLogger.logConfig(
        'testSetting',
        'oldValue',
        'newValue',
      ), returnsNormally);
    ...
    test('LoggingManager update configuration works', () {
      ...
      LoggingManager.updateConfig((config) => config.copyWith(level: Level.WARNING));
